.PHONY: build run stop test logs health clean run-azure stop-azure run-huawei stop-huawei run-gcp stop-gcp start-all stop-all
...
start-all: run run-azure run-huawei run-gcp
	@echo "All clouds started"

stop-all: stop stop-azure stop-huawei stop-gcp
	@echo "All clouds stopped"

run-azure:
	@echo "Starting Azure..."
	docker compose -f docker-compose.azure.yml up -d --build
	@echo "Azure running on http://localhost:$(PORT)"
	@echo "Health: http://localhost:$(PORT)/_azure/health"
...
run-huawei:
	@echo "Starting Huawei Cloud..."
	docker compose -f docker-compose.huawei.yml up -d --build
	@echo "Huawei Cloud running on http://localhost:$(PORT)"
	@echo "Health: http://localhost:$(PORT)/_huawei/health"

stop-huawei:
	docker compose -f docker-compose.huawei.yml down 2>/dev/null || true
	@echo "Huawei Cloud stopped"

run-gcp:
	@echo "Starting GCP..."
	docker compose -f docker-compose.gcp.yml up -d --build
	@echo "GCP running on http://localhost:$(PORT)"
	@echo "Health: http://localhost:$(PORT)/_gcp/health"
